    ...
    <header className="sticky top-0 z-40 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 text-slate-800 dark:text-slate-100 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          {/* Right Controls: Role Switcher, Dark/Light Mode Toggle, Language Toggle, Notification, Profile */}
          <div className="flex items-center space-x-2 sm:space-x-2.5 rtl:space-x-reverse">
            ...
            <div className="relative">
              ...
              {roleDropdownOpen && (
                <div
                  className="absolute top-full mt-2 w-56 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl py-2 z-50 animate-in fade-in slide-in-from-top-2"
            ...
            <button
              onClick={toggleTheme}
              className="p-2 rounded-xl text-slate-600 dark:text-slate-300 hover:text-emerald-600 dark:hover:text-emerald-400 hover:bg-slate-100 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-700/60 transition-all"
            ...
            <button
              ...
              className="flex items-center space-x-1.5 rtl:space-x-reverse px-2.5 py-1.5 rounded-xl text-xs font-semibold text-slate-700 dark:text-slate-300 hover:text-slate-950 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-700/60 transition-colors"
            ...
            <div className="relative">
              ...
              {userDropdownOpen && (
                <div
                  className="absolute top-full mt-2 w-48 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl py-2 z-50"
                  style={{ [dir === "rtl" ? "left" : "right"]: 0 }}
                >
                  ...
                  <Link
                    ...
                    className="block px-4 py-2 text-xs text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700/50"
                  >
                    {t("clientProfile")}
                  </Link>
                  ...
                  <button
                    ...
                    className="w-full text-start px-4 py-2 text-xs text-red-500 dark:text-red-400 hover:bg-slate-100 dark:hover:bg-slate-700/50 border-t border-slate-200 dark:border-slate-700/40"
                  >
                    {t("navLogout")}
                  </button>
                </div>
