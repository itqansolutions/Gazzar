    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-5 sm:p-6 shadow-xl">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-sm font-bold text-white">{t("measurementsHistory")}</h3>
            <p className="text-xs text-slate-400">{language === "ar" ? "رسم بياني لتغير الوزن ونسبة الدهون عبر الزمن" : "Weight & Body Fat chart over time"}</p>
          </div>
          <button
            onClick={onOpenModal}
            className="px-3 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs flex items-center space-x-1 rtl:space-x-reverse"
          >
        ...
        <div className="h-64 sm:h-72 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData}>
              ...
              <Area type="monotone" dataKey="weight" name="الوزن (KG)" stroke="#10b981" strokeWidth={3} fill="url(#weightGradClient)" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="bg-slate-900/90 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
        <div className="p-4 border-b border-slate-800 flex items-center justify-between">
          <h3 className="text-xs font-bold text-white uppercase tracking-wider">سجل القياسات التفصيلي</h3>
        </div>
            ...
            <tbody className="divide-y divide-slate-800/60 text-slate-300">
              {measurements.map(m => (
                <tr key={m.id} className="hover:bg-slate-800/30 transition-colors">
                  ...
                  <td className="p-3 text-center font-bold text-emerald-400">{m.weightKg} KG</td>
                  <td className="p-3 text-center">{m.bmi}</td>
                  <td className="p-3 text-center">{m.bodyFatPercentage}%</td>
                  <td className="p-3 text-center">{m.muscleMassKg} KG</td>
                  <td className="p-3 text-center">{m.waistCm} CM</td>
                  <td className="p-3 text-center">{m.chestCm} CM</td>
                  <td className="p-3 text-center">{m.armsCm} CM</td>
